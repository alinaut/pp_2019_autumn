...
rgb* linear_filter_with_gauss(int n, int m, int code) {
    ...
    if (rank == 0) {
        if (code == 0) {
            source = createImageOne(n, m);
        } else {
            source = createImageTwo(n, m);
        }
        imgCols = m;
        imgRows = n;
        result = new rgb[n*m];
        ...
        int colSize = std::max(imgRows / size, 1);
        for (int i = 0; i < std::min(imgRows, size - 1); i++) {
        ...
        if (size == 1) {
            ...
            rows_per_process[std::min(imgRows, size) - 1] = std::max(0,
            n - colSize*(std::min(imgRows, size) - 1));
            displacements[std::min(imgRows, size) - 1] = displacements[std::min(imgRows, size) - 2] +
            send_counts[std::min(imgRows, size) - 2];
            send_counts[std::min(imgRows, size) - 1] =
            rows_per_process[std::min(imgRows, size) - 1]*imgCols*3;
        }
    }
