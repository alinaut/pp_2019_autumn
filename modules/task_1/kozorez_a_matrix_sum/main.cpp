...
TEST(Parallel_Sum_MPI, Test_Matrix_Sum) {
    ...
    int global_sum = getParallelSum(matrix, n, m);
...
TEST(Parallel_Sum_MPI, Test_not_Positive) {
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    std::vector<int> matrix;
    const int n = -4;
    ...
    if (rank == 0) {
        ASSERT_ANY_THROW(getRandomMatrix(m, n));
    }
}
...
TEST(Parallel_Sum_MPI, Test_Vector_and_Matrix_Size_Different) {
    ...
    ASSERT_ANY_THROW(getParallelSum(matrix, n, m));
}
